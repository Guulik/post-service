...
func (c *CommentsObservers) CreateObserver(postId int) (int, chan *model.Comment, error) {
...
func (c *CommentsObservers) NotifyObservers(postId int, comment model.Comment) error {
	c.mu.Lock()
	defer c.mu.Unlock()
	...
	if ok {
		...
		return errors.New(constants.ThereIsNoObserversError)
	}
